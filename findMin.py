...
def updateCapRanges(capRanges, capNeeded):
    ...
    for capType in capRanges:
        if(capType in capNeeded):
            ...
            CAP_PERCENT = round(1 - capRanges[capType]["recent"]/max(POSSIBLE_CAPS), 4)
...
def sameLists(a, b):
    """
    Determines if two lists have the same values in them
    Inputs: a=list(str), b=list(str)
    Returns: bool
    """
    sameLen = len(a) == len(b)
    aInB = all([i in b for i in a])
    bInA = all([i in a for i in b])
    return sameLen and aInB and bInA

def findStartingCap(slots, toCraft, materials):
    """
    Find the minimum equal carry capacity to craft recipes
    """
    for cap in range(max(POSSIBLE_CAPS), 9, -1):
        # Setup inventory
        inv = { 
...
for anvilTab in ANVIL_TABS:
    for ti, t in enumerate(TIERS):
        ...
        if(len(CRAFTABLE) >= t and t < max(ALL_SLOTS)):
            ...
            if(carryAndCraft):
                # Determine if recipes are a repeat
                for known in RECOMMENDATIONS.values():
                    if(sameLists(a=ccRecipes, b=known["recipes"])):
                        print("Repeating known recipes!")
                        repeatRecipes = True
                        RECOMMENDATIONS[f"anvil{anvilTab}tier{ti}"] = deepcopy(known)
                if(not repeatRecipes):
